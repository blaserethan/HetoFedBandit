...
from lib.FederatedOneModelNew import HetoFedBandit_Simplified, HetoFedBandit_Time_Recluster, HetoFedBandit_Data_Recluster
...
class experimentOneRealData(object):
    def __init__(self, namelabel, dataset, context_dimension, batchSize=25, plot=True, Write_to_File=False):
        ...
        if self.dataset == 'LastFM':
            ...
            self.event_fileName = self.address + "/ordered_time_ordered_users.dat"
    ...
    def runAlgorithms(self, algorithms, startTime):
        ...
        with open(self.event_fileName, 'r') as f:
            ...
            for _, line in enumerate(f, 1):
                ...
                if (not algorithms['HetoFedBandit_Simplified'].explore_phase and (iter_ % config["n_users"] == 0)):
...
if __name__ == '__main__':
    ...
    if not args.alg:
        ...
        algorithms['NIndepLinUCB-alpha=0.3'] = SyncLinUCB(dimension=config["context_dimension"], alpha=0.9,
                                            lambda_=config["lambda_"],
                                            delta_=1e-1,

...
# in this algorithm there is no exploration phase and we dynamically recluster every time someone requests collaboration
class HetoFedBandit_Recluster_PQ(HetoFedBandit_PQ):
    def __init__(self, dimension, alpha, lambda_, delta_, NoiseScale, threshold, exploration_length, neighbor_identification_alpha):
        super().__init__(dimension, alpha, lambda_, delta_, NoiseScale, threshold, exploration_length, neighbor_identification_alpha)
...
# in this algorithm there is not exploration phase and we dynamically recluster every time someone requests collaboration
class HetoFedBandit_Recluster_FIFO(HetoFedBandit_Simplified):
    def __init__(self, dimension, alpha, lambda_, delta_, NoiseScale, threshold, exploration_length, neighbor_identification_alpha):
        super().__init__(dimension, alpha, lambda_, delta_, NoiseScale, threshold, exploration_length, neighbor_identification_alpha)
        self.explore_phase = False  
...
class HetoFedBandit_Time_Recluster(HetoFedBandit_Recluster_FIFO):
    ...
    def homogeneityTest(self, currentUser, neighborUser):
        ...
        df1 = int(currentUser.rank + neighborUser.rank - XCombinedRank)
        chiSquareStatistic = num / (self.NoiseScale**2)
        psi = max(n,m) / ((self.T)*self.NoiseScale**2* len(self.clients)**2)
...
class HetoFedBandit_Data_Recluster(HetoFedBandit_Recluster_FIFO):
    def __init__(self, dimension, alpha, lambda_, delta_, NoiseScale, threshold, exploration_length, neighbor_identification_alpha,T):
        super().__init__(dimension, alpha, lambda_, delta_, NoiseScale, threshold, exploration_length, neighbor_identification_alpha)
        self.explore_phase = False
        self.T = T
    ...
    def homogeneityTest(self, currentUser, neighborUser):
        ...
        psi = w.max() / ((self.T)*self.NoiseScale**2* len(self.clients)**2)
